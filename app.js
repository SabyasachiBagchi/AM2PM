  ...
  class FoodServiceApp {
    ...
    startClock() {
      const update = () => {
        ...
        document.getElementById('clock-date').textContent = now.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
      };
      update();
    ...
    showApp(isAdmin) {
      ...
      select.innerHTML = this.users
        .map(u => `<option value="${u}" ${u === this.currentUser ? 'selected' : ''}>${u}</option>`)
        .join('');
      ...
      this.mainContent.innerHTML = `
        <div id="dashboardView" class="view">
          <div class="dashboard-header">
            ...
            <div class="month-nav">
              <button id="prevMonth" class="btn btn--outline"><i class="fas fa-chevron-left"></i></button>
              <span id="currentMonth">${this.currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
              <button id="nextMonth" class="btn btn--outline"><i class="fas fa-chevron-right"></i></button>
            </div>
          </div>
          ...
          <div class="stats-grid">
            <div class="stat-card glass-effect">
              ...
              <div class="stat-content"><h3>₹${stats.balanceRemaining}</h3><p>Balance Remaining</p></div>
            </div>
            <div class="stat-card glass-effect">
              <div class="stat-icon"><i class="fas fa-rupee-sign"></i></div>
              <div class="stat-content"><h3>₹${stats.spentThisMonth}</h3><p>Spent This Month</p></div>
            </div>
            <div class="stat-card glass-effect">
              <div class="stat-icon"><i class="fas fa-credit-card"></i></div>
              <div class="stat-content"><h3>₹${stats.paidThisMonth}</h3><p>Paid This Month</p></div>
            </div>
            <div class="stat-card glass-effect">
              <div class="stat-icon"><i class="fas fa-utensils"></i></div>
              <div class="stat-content"><h3>${stats.mealsThisMonth}</h3><p>Meals This Month</p></div>
            </div>
          </div>
    ...
    renderCalendar() {
      ...
      const startDay = (firstDay.getDay() + 6) % 7; // Monday-first alignment
      ...
      this.mainContent.innerHTML = `
        <div id="dayDetailView" class="view">
          <div class="day-detail-header">
            <button id="backToCalendar" class="btn btn--outline"><i class="fas fa-arrow-left"></i> Back</button>
            <h1>${this.selectedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</h1>
          </div>
          ...
          <div class="meal-cards">
            <div class="meal-card glass-effect">
              ...
              <div class="meal-toggle">
                <label class="toggle-switch">
                  <input type="checkbox" id="lunchToggle" ${dayData.lunch ? 'checked' : ''} ${!this.isAdmin ? 'disabled' : ''}>
            ...
            <div class="meal-card glass-effect">
              ...
              <div class="meal-toggle">
                <label class="toggle-switch">
                  <input type="checkbox" id="dinnerToggle" ${dayData.dinner ? 'checked' : ''} ${!this.isAdmin ? 'disabled' : ''}>
                  <span class="toggle-slider"></span>
                </label>
          ...
          <div class="day-summary">
            <div class="summary-card glass-effect">
              <h4>Daily Summary</h4>
              <div class="summary-item"><span>Meals:</span><span>${mealCount}</span></div>
              <div class="summary-item"><span>Cost:</span><span>₹${mealCount * this.mealRate}</span></div>
            </div>
          </div>
        </div>`;
    }
    ...
    renderPaymentView() {
      this.mainContent.innerHTML = `
        <div id="paymentView" class="view">
          ...
          <div class="payment-history glass-effect">
    ...
    handlePaymentFormSubmit() {
      ...
      if (this.editingPaymentId) {
        const idx = this.paymentData[this.currentUser].findIndex(p => p.id == this.editingPaymentId);
        if (idx > -1) {
          this.paymentData[this.currentUser][idx].amount = amount;
          this.paymentData[this.currentUser][idx].date = date;
        }
      } else {
    ...
    resetPaymentForm() {
      ...
      formContainer.innerHTML = `
        <h3 id="paymentFormTitle">Record Payment</h3>
        <form id="paymentForm">
    ...
    updatePaymentList() {
      ...
      if (payments.length === 0) {
        listEl.innerHTML = '<p>No payments recorded.</p>';
        return;
      }
          ...
          listEl.innerHTML += `
            <div class="payment-item">
              ...
              <div class="payment-actions">
                ${this.isAdmin ? `
                  <button class="btn--icon edit-btn" data-payment-id="${p.id}"><i class="fas fa-edit"></i></button>
                  <button class="btn--icon delete-btn" data-payment-id="${p.id}"><i class="fas fa-trash"></i></button>
                ` : ''}
              </div>
            </div>`;
        });
